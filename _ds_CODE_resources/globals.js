	...
	if (application.__parent__.solutionPrefs && !solutionPrefs.config.lockStatus) {
		...
		if (setDefault) {
			...
			if (navigationPrefs.byNavItemID[currentNavItem].fastFind && navigationPrefs.byNavItemID[currentNavItem].fastFind.lastFindField) {
				...
				var prettyFind = navigationPrefs.byNavItemID[currentNavItem].fastFind ? navigationPrefs.byNavItemID[currentNavItem].fastFind.filter(function(item){return item.columnName == findInitial}) : new Array()
 ...
 * @param 	{Boolean} 	[workflow] Lock workflow only (no pressing any buttons when no records, for example)
 *
 * @properties={typeid:24,uuid:"8a2db575-a9de-4646-9936-14468a01d7f4"}
 */
function TRIGGER_interface_lock(freeze,freezeAll,nonTransparent,spinner,nonTransparentText,oldMode,workflow) {
//TODO:
	...
	var workflow = arguments[6]

	//check to see that solutionPrefs is defined and parameter passed
	if (application.__parent__.solutionPrefs) {
		if (typeof freeze == 'boolean' || typeof workflow == 'boolean') {
			var baseForm = solutionPrefs.config.formNameBase
			...
			if (solutionPrefs.config.webClient) {
				if (typeof workflow == 'boolean') {
					forms.DATASUTRA_WEB_0F__workflow.elements.gfx_curtain.visible = workflow
					solutionPrefs.design.statusLockWorkflow = workflow
				}
				//only implements freeze for now
				else {
...
function TRIGGER_progressbar_start(progressValue,explanationText,explanationToolTip,minimum,maximum) {
	if (application.__parent__.solutionPrefs) {
		...
		//indeterminate gif
		if (progressValue == -273 || (application.getApplicationType() == APPLICATION_TYPES.WEB_CLIENT && progressValue == null && typeof progressValue == 'object')) {
			forms[formName].elements.bean_progress.visible = false
...
/**
 * Copy to clipboard in smart/web clients
 *
 * @param {String}	text String to copy to clipboard
 *
 * @properties={typeid:24,uuid:"64981D7D-CF4A-48A9-A87A-B8F039A3AC88"}
 */
function CODE_clipboard_set(text) {
	if (typeof text == 'string') {
		if (application.getApplicationType() == APPLICATION_TYPES.WEB_CLIENT) {
			plugins.WebClientUtils.executeClientSideJS('window.prompt("Copy to Clipboard: Ctrl+C (Windows) or Cmd+C (Mac) and then press Enter", "' + text +'");')
		}
		else {
			application.setClipboardContent(text)
		}
	}
}